...
async function loadProjects() {
  ...
  try {
    ...
    console.error(e);
    showToast(e.message || 'Error cargando proyectos', 'error');
  } finally {
    showLoading(false);
  }
}
...
async function deleteProject(id) {
  ...
  if (!resp.ok && resp.status !== 204) {
    ...
    throw new Error(text || `Error ${resp.status} al eliminar`);
  }
  return true;
}
  ...
  if (action === 'delete') {
    if (confirm('¿Eliminar este proyecto?')) {
      try {
        ...
        console.error(e2);
        showToast(e2.message || 'Error eliminando proyecto', 'error');
      }
    }
  }
});
...
form?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    console.error(e2);
    showToast(e2.message || 'Error guardando proyecto', 'error');
  }
});
